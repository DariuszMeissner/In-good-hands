...
export const HelpCategoryListing = ({ start, end, helpData, isActive }) => {
    return (
        <div className='container'>
            <ul className='help-category-listing row p-l-0-s m-t-8 p-r-0-s p-l-20-l p-r-20-l'>
                {helpData.map(el =>
                    el.category === isActive
                        ?
                        <li key={el.category} className='c-s-12'>
                            {el.companies.slice(start, end).map(el =>
                                <div className='help-category-item flex flex-column-s flex-row-ml flex-justify-between' key={el.name}>
                                    <div>
